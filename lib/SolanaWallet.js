...
const TOKEN_ENCODE = {
  [sol.TOKEN_PROGRAM]: sol.PROGRAMS.Token.token.instructions.encoders,
  [sol.TOKEN_PROGRAM2022]: sol.PROGRAMS.Token2022['token-2022'].instructions.encoders,
};
const ASSOCIATED_TOKEN_ENCODE = {
  [sol.TOKEN_PROGRAM]: sol.PROGRAMS.Token.associatedToken.instructions.encoders,
  [sol.TOKEN_PROGRAM2022]: sol.PROGRAMS.Token2022.associatedToken.instructions.encoders,
};
const ACCOUNT_SIZES = {
  [sol.TOKEN_PROGRAM]: sol.PROGRAMS.Token.token.accounts.coders.token.size,
  [sol.TOKEN_PROGRAM2022]: sol.PROGRAMS.Token2022['token-2022'].accounts.coders.token.size,
};
...
export default class SolanaWallet extends CsWallet {
  ...
  async _getMinerFee() {
    ...
    const tx = this.crypto.type === 'coin'
      ? sol.createTransferSol(this.#account, this.#account, 1n, blockhash, 'legacy')
  ...
  async createTransaction({ address, amount, price }, seed) {
    ...
    const unsignedTx = sol.createTx(this.#account, instructions, blockhash, 'legacy');
