...
export default class SolanaWallet extends CsWallet {
  ...
  get dummyExchangeDepositAddress() {
    return sol.getAddress(ed25519.utils.randomPrivateKey());
  }
  ...
  async open(publicKey) {
    ...
    if (publicKey.settings.bip44 === this.settings.bip44) {
      this.#account = sol.getAddressFromPublicKey(hex.decode(publicKey.data));
      this.#init();
      this.state = CsWallet.STATE_INITIALIZED;
    } else {
  ...
  #getAssociatedTokenAccount(mint, owner, allowOffCurveOwner = false) {
    try {
      return sol.tokenAddress(mint, owner, allowOffCurveOwner);
    } catch (err) {
  ...
  async _getRent(type = this.crypto.type) {
    return this.#api.getRent((type === 'token') ? ACCOUNT_SIZE : 0);
  }
  ...
  async createTransaction({ address, amount, price }, seed) {
    ...
    const res = sol.signTx(this.#keypairFromSeed(seed).privateKey, transaction);
    const id = await this.#api.sendTransaction(res[1]);
    if (this.crypto.type === 'coin') {
    ...
    if (tx.meta.postTokenBalances) {
      for (const postTokenBalance of tx.meta.postTokenBalances) {
        postTokenBalances.push({
          ...postTokenBalance,
          account: this.#getAssociatedTokenAccount(postTokenBalance.mint, postTokenBalance.owner, true),
        });
      }
    }
